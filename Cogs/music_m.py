...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    @classmethod
    async def create_source(cls, ctx: SlashContext, search: str, *, loop, download=False):
...
class Music(commands.Cog):
    ...
    async def __local_check(self, ctx: SlashContext):
        """A local check which applies to all commands in this cog."""
        if not ctx.guild:
            raise commands.NoPrivateMessage
        return True

    async def __error(self, ctx: SlashContext, error):
    ...
    async def connect_(self, ctx: SlashContext):
        try:
